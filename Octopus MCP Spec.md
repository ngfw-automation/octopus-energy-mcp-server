...
| `get_standing_charges` | Standing-charge history, per payment method (rows carry `value_exc_vat`/`value_inc_vat`, **not** a bare `value`; an account whose bill differs can pin `STANDING_CHARGE_*` in VAT-inclusive p/day, which then takes precedence) | `product_code`,`tariff_code`,`period_from?`,`period_to?`,`payment_method?` | charges[] (+configured value) | REST standing-charges |
